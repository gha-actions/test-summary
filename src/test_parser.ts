...
async function parseJunitXml2(xml: any): Promise<TestResult> {
    const suites: TestSuite[] = [ ]
    const counts = {
        passed: 0,
        failed: 0,
    ...
    if(xml.testExecutions && xml.testExecutions.file){
        const files = xml.testExecutions.file
        for (const file of files) {
            const cases = [ ]

            if (!Array.isArray(file.testCase)) {
                continue
            }

            for (const testcase of file.testCase) {
                let status = TestStatus.Pass
...
export async function parseFile(filename: string): Promise<TestResult> {
    ...
    }else if(xml.testExecutions){
        return await parseJunitXml2(xml)
    }
